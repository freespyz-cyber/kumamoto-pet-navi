setTimeout(() => {
  ...
  const enhancePopup=()=>{const popup=document.querySelector('.leaflet-popup-content');if(!popup||popup.querySelector('.map-add-plan'))return;const html=popup.innerHTML;const cat=html.match(/分類：([^<]+)/)?.[1]||'';const tags=cat==='病院'?'<span class="map-tag">診療対応</span><span class="map-tag">要確認</span><span class="map-tag">来院前確認</span>':cat.includes('食事')?'<span class="map-tag">犬同伴可</span><span class="map-tag">テラス席など</span><span class="map-tag">来店前確認</span>':cat.includes('ドッグラン')?'<span class="map-tag">犬OK</span><span class="map-tag">ドッグラン</span><span class="map-tag">要確認</span>':'<span class="map-tag">公式情報確認</span>';popup.insertAdjacentHTML('beforeend','<br>'+tags+'<br><button class="map-add-plan">この場所を予定に追加</button>');popup.querySelector('.map-add-plan').onclick=()=>{const n=popup.querySelector('strong')?.textContent||'この場所';if(!selected.includes(n))selected.push(n);plan.textContent='自分の予定を作る：'+selected.join('　・　');popup.querySelector('.map-add-plan').textContent='追加済み'}};
  new MutationObserver(enhancePopup).observe(mapEl,{childList:true,subtree:true});
  enhancePopup();
}, 1200);
